...
from .SubtitlistUtilities import get_language_info
from ..utilities import log

import re
from html.parser import HTMLParser
HDR = {'Host': 'www.subtitlist.com',
...
main_url = "https://www.subtitlist.com"
debug_pretext = "subtitlist.com"


subtitlist_languages = {
...
def getSearchTitle(title, year=None):  # new Add
    ...
    list1 = []
    for block in blocks:
        regx = '*<a href="(.*?)" class="item-author text-color">(.*?)</a>'
        try:
            ...
            print(("hrefxxx", href))
            print(("yearxx", year))
            href = 'https://www.subtitlist.com' + href
            if year and year == '':
            ...
            break
    return 'https://www.subtitlist.com/subtitles/%s' % hrf
...
def get_subtitles_list(searchstring, title, year, languageshort, languagelong, subtitles_list):
    ...
    for subtitle in subtitles:
        try:
            filename = re.compile('<a.+?</span>(.+?)</a>').findall(subtitle)[0]
            filename = filename.strip()
            id = re.compile('<a href="(.+?)" title').findall(subtitle)[0]
            #print(id)
            try:
                downloads = re.compile('<a href="(.+?)" title').findall(subtitle)[0]
